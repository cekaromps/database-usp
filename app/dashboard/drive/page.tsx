...
export default function CloudDrive() {
  ...
  try {
    const response = await fetch(`/api/drive/file?filePath=${encodeURIComponent(fullPath)}`, {
      method: 'DELETE',
    });
  ...
  const createFolder = async () => {
    ...
    try {
      await fetch('/api/drive', {
        ...
        body: JSON.stringify({ currentPath, folderName: newFolderName }),
      });
      setNewFolderName('');
      loadFiles();
    } catch (err) {
      ...
      <div className="w-full h-full bg-macos-base rounded-xl border border-macos-separator shadow-2xl flex flex-col overflow-hidden">
        ...
        <div className="h-14 bg-macos-secondary border-b border-macos-separator flex items-center justify-between px-4 select-none">
          <div className="flex items-center gap-1.5 text-sm font-medium text-macos-primary max-w-md truncate">
            ...
            {pathSegments.map((segment, index) => {
              const buildPath = pathSegments.slice(0, index + 1).join('/');
              return (
        ...
        <div className="flex-1 flex overflow-hidden">
          ...
          <div className="w-48 bg-macos-secondary border-r border-macos-separator p-3 flex flex-col">
            ...
            />
            <label onClick={createFolder} className="mb-3 w-full bg-macos-tertiary border border-macos-separator text-macos-primary hover:bg-macos-popover text-xs font-medium py-2 px-3 rounded-md flex items-center justify-center gap-2 cursor-pointer transition">
            ...
            <div className="flex-1 p-6 overflow-y-auto flex flex-col">
              {items.length === 0 ? (
                <div className="w-full h-full flex flex-col items-center justify-center text-center">
                  ...
                  <p className="text-sm font-medium text-macos-secondary">Empty Directory</p>
                  <p className="text-xs text-macos-tertiary mt-0.5">Drag files or use the upload button</p>
                </div>
              ) : (
                    ...
                    >
                      {item.isDir ? (
                        ...
                        <>
                          ...
                          >
                            <div className="text-xl mr-3 drop-shadow-md transform group-hover:scale-105 transition-transform duration-150">📁</div>
