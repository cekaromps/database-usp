...
interface ItemRow {
  ...
  qty: number
  amountIdr: number
  processes: string[] // 👈 Tambahkan array pelacak proses terpilih
}
...
export default function InvoiceForm() {
  ...
  // Inisialisasi awal dengan properti array processes kosong []
  ...
  const handleInputChange = (value: string) => {
    ...
    if (cleanInput.length > 0) {
      const filtered = Object.keys(CUSTOMER_CODES).filter(name => name.toUpperCase().includes(cleanInput))
  ...
  const addItemRow = () => {
    setItems([...items, { id: Date.now(), description: "", qty: 1, amountIdr: 0, processes: [] }])
  }
  ...
  // 🌟 FUNGSIONALITAS TOGGLE CHECKBOX PROSES KERJA BARU
  const handleToggleProcess = (itemId: number, processName: string) => {
    setItems(items.map((item) => {
      ...
      const updatedProcesses = isExist
        ...
        : [...item.processes, processName] // Tambah jika di-check
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault()
    setError(null)
    if (items.some(item => item.amountIdr <= 0 || item.qty <= 0)) {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      {error && (
        <div className="p-3 bg-macos-red/10 border border-macos-red/30 text-macos-red rounded-lg text-sm font-medium">✕ {error}</div>
      )}
      ...
      <div className="text-[11px] font-mono bg-macos-tertiary border border-macos-separator p-2 rounded-md text-macos-secondary flex items-center justify-between">
        ...
        <span className="text-macos-blue font-bold">{customerName ? `Membaca riwayat "${customerName}"...` : "Menunggu pilihan Customer..."}</span>
      </div>
      ...
      <div className="bg-macos-popover border border-macos-separator p-6 rounded-xl shadow-2xl space-y-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div ref={autocompleteRef} className="relative">
            ...
            <input name="customer" type="text" required autoComplete="off" value={customerName} disabled={isPending} placeholder="Ketik nama perusahaan, cth: ALCO..." onFocus={() => setShowSuggestions(true)} onChange={(e) => handleInputChange(e.target.value)} className="w-full bg-macos-tertiary border border-macos-separator text-macos-primary rounded-md p-2 text-sm focus:outline-none focus:border-macos-blue" />
            {showSuggestions && suggestions.length > 0 && (
              <div className="absolute left-0 right-0 mt-1 max-h-48 overflow-y-auto bg-macos-popover border border-macos-separator rounded-lg shadow-2xl z-50 py-1 divide-y divide-macos-separator/40 animate-scale-up">
                {suggestions.map((name) => (
                  <button key={name} type="button" onClick={() => handleSelectSuggestion(name)} className="w-full text-left px-3 py-2 text-xs text-macos-primary hover:bg-macos-blue hover:text-white font-medium border-none bg-transparent">🏢 {name}</button>
                ))}
              </div>
            )}
          </div>
          <div>
            ...
            <input name="quotationNumber" type="text" required readOnly value={quotationNumber} className="w-full bg-macos-separator/30 border border-macos-separator text-macos-secondary font-mono rounded-md p-2 text-sm select-none cursor-not-allowed font-semibold opacity-70" />
          </div>
          <div>
      ...
      {/* ITEMS DYNAMIC SECTION WITH PROCESSES CHECKBOXES */}
      <div className="bg-macos-popover border border-macos-separator p-6 rounded-xl shadow-2xl space-y-4">
        <div className="flex items-center justify-between border-b border-macos-separator pb-2 mb-2">
          ...
          <button type="button" onClick={addItemRow} disabled={isPending} className="px-3 py-1.5 bg-macos-blue/10 border border-macos-blue/30 text-macos-blue rounded-md text-xs font-semibold hover:bg-macos-blue hover:text-white transition cursor-pointer">＋ Add Item Description</button>
        </div>
        
        <div className="space-y-4">
          {items.map((item, index) => (
            <div key={item.id} className="bg-macos-base/30 p-5 border border-macos-separator/40 rounded-xl space-y-4 animate-scale-up">
              {/* Baris Atas: Input Data */}
              <div className="flex flex-col sm:flex-row sm:items-end gap-4">
                <div className="flex-1 w-full">
                  <label className="block text-xs font-medium text-macos-secondary mb-1.5">Item Description #{index + 1} *</label>
                  <input type="text" required value={item.description} disabled={isPending} onChange={(e) => updateItemField(item.id, "description", e.target.value)} placeholder="e.g. SHAFT COUPLING MACHINING" className="w-full bg-macos-tertiary border border-macos-separator text-macos-primary rounded-md p-2 text-sm focus:outline-none focus:border-macos-blue" />
                </div>
                
                <div className="w-full sm:w-24">
                  <label className="block text-xs font-medium text-macos-secondary mb-1.5">Qty *</label>
                  <input type="number" min="1" required value={item.qty} disabled={isPending} onChange={(e) => updateItemField(item.id, "qty", parseInt(e.target.value, 10) || 1)} className="w-full bg-macos-tertiary border border-macos-separator text-macos-primary rounded-md p-2 text-sm focus:outline-none focus:border-macos-blue" />
                </div>

                <div className="w-full sm:w-44">
                  <label className="block text-xs font-medium text-macos-secondary mb-1.5">Amount IDR *</label>
                  <input type="number" step="any" required value={item.amountIdr || ""} disabled={isPending} onChange={(e) => updateItemField(item.id, "amountIdr", parseFloat(e.target.value) || 0)} placeholder="3500000" className="w-full bg-macos-tertiary border border-macos-separator text-macos-primary rounded-md p-2 text-sm focus:outline-none focus:border-macos-blue" />
                </div>

                {items.length > 1 && (
                  <button type="button" onClick={() => removeItemRow(item.id)} disabled={isPending} className="w-full sm:w-auto p-2 mb-0.5 border border-macos-red/20 text-macos-red bg-macos-red/5 rounded-md text-xs hover:bg-macos-red hover:text-white transition flex justify-center">✕</button>
                )}
              </div>
              ...
              <div className="border-t border-macos-separator/30 pt-3">
                ...
                <div className="flex flex-wrap gap-x-5 gap-y-2">
                  {AVAILABLE_PROCESSES.map((process) => {
                    const isChecked = item.processes.includes(process)
                    return (
                      <label key={process} className="flex items-center gap-2 text-xs font-medium text-macos-primary cursor-pointer select-none group">
                        <input 
                          type="checkbox"
                          disabled={isPending}
                          checked={isChecked}
                          onChange={() => handleToggleProcess(item.id, process)}
                          className="w-3.5 h-3.5 rounded border border-macos-separator bg-macos-tertiary text-macos-blue focus:ring-0 focus:ring-offset-0 transition accent-macos-blue cursor-pointer"
                        />
                        <span className={`group-hover:text-white transition ${isChecked ? "text-macos-blue font-bold" : "text-macos-secondary"}`}>
                          {process}
                        </span>
                      </label>
                    )
      ...
      <div className="bg-macos-popover border border-macos-separator p-6 rounded-xl shadow-2xl space-y-4">
        <div>
          ...
          <input name="remark" type="text" disabled={isPending} placeholder="Prices are valid 1 month after offer is sent" className="w-full bg-macos-tertiary border border-macos-separator text-macos-primary rounded-md p-2 text-sm focus:outline-none focus:border-macos-blue" />
        </div>
        <div className="pt-2">
          <button type="submit" disabled={isPending} className="w-full py-2.5 bg-macos-blue text-white rounded-md font-semibold text-sm hover:bg-opacity-90 active:scale-[0.99] transition shadow-lg disabled:opacity-50">
