  ...
  return (
    <div className="min-h-screen bg-neutral-200 py-8 px-4 font-sans antialiased print:bg-white print:p-0 print:min-h-0">
      ...
      <div className="print-hidden-action max-w-[210mm] mx-auto mb-6 bg-white p-4 rounded-xl shadow border border-neutral-300 flex items-center justify-between">
        ...
        <a 
          href="javascript:window.print()" // 👈 Menggunakan tautan native murni yang aman dari error Server Component
      ...
      <div className="a4-container text-black">
            ...
            <tr>
              ...
              <td className="meta-box">
                ...
                <div className="flex gap-2 mt-2 pt-2 border-t border-neutral-200/50">
                  <span className="font-bold text-[10px] w-10 shrink-0">Attn &nbsp;&nbsp;&nbsp;:</span>
                  <p className="text-neutral-900 text-[10px]">{mainData.attn}</p>
                </div>
                <div className="flex gap-2 mt-1">
                  <span className="font-bold text-[10px] w-10 shrink-0">CC &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;:</span>
                  <p className="text-neutral-900 text-[10px]">{mainData.cc}</p>
                </div>
              </td>
              ...
              <td className="meta-box">
                <div className="flex justify-between items-start w-full mb-2 pb-2 border-b border-neutral-200/50">
                  ...
                  <div className="text-right shrink-0">
                    <p className="text-neutral-500 text-[9px] leading-none">Handphone :</p>
                    <p className="font-black text-neutral-950 text-[10px] mt-0.5">{mainData.handphone}</p>
                  </div>
                </div>
                <div className="grid grid-cols-3 gap-y-0.5 text-[10px]">
                  <span className="font-bold text-neutral-600">Term</span>
                  <span className="col-span-2">: {mainData.term}</span>
                  <span className="font-bold text-neutral-600">Validity</span>
                  <span className="col-span-2">: {mainData.validity}</span>
                  <span className="font-bold text-neutral-600">Lead Time</span>
                  <span className="col-span-2">: {mainData.leadTime}</span>
                </div>
              </td>
            </tr>
          </tbody>
          ...
          <table className="main-goods-table">
            <thead>
              <tr className="border-b border-black font-bold text-center bg-neutral-50 text-[11px]">
                ...
                <th className="p-2 border-r border-black w-16">Part No</th>
            ...
            <tbody className="divide-y divide-neutral-300">
              {invoiceItems.map((item, index) => (
                <tr key={item.id} className="item-row">
                  ...
                  <td className="border-r border-neutral-300 text-right font-mono text-neutral-800">{formatIDR(item.amountIdr)}</td>
                  <td className="text-right font-mono font-bold text-neutral-950">{formatIDR(item.amountIdr * item.qty)}</td>
                </tr>
              ))}
              
              {/* BARIS PENGISI KOSONG (PENYEIMBANG TINGGI KERTAS A4) */}
              <tr className="h-36">
        ...
        <div className="border border-black p-3 rounded-md text-[11px] font-bold text-neutral-900 mb-5 bg-neutral-50/50">
          Remarks : <span className="font-medium italic text-neutral-700">{mainData.remark}</span>
        </div>
        ...
        <div className="footer-section flex justify-between items-end text-[11px] pt-4">
          <div className="text-neutral-800 space-y-1">
            ...
            <p>Best Regards</p>
            <p className="italic text-neutral-400 text-[10px] pt-4">This Quotation is computer generated and requires no signature</p>

            <p className="font-bold text-neutral-800 uppercase text-[10px]">PT UTAMA PASOGIT SEJAHTERA</p>
          </div>
        </div>
