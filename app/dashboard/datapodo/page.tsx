...
export default async function DataPoDoPage({ searchParams }: PageProps) {
  ...
  // Ambil data dari Prisma dengan filter pencarian
  ...
  // Server Action Bridge untuk memproses penghapusan data aman di sisi server
    ...
    <div className="min-h-screen bg-macos-base text-macos-primary p-10 font-sans antialiased">
      ...
      <div className="flex items-center justify-between mb-8 pb-4 border-b border-macos-separator">
        ...
        <Link 
          ...
          title="Kembali ke Dashboard Central"
        >
          <ArrowLeft size={18} />
        </Link>
          <div className="flex items-center gap-4">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-10">
        <div className="lg:col-span-2 bg-macos-popover border border-macos-separator p-6 rounded-xl shadow-2xl">
          <h3 className="text-lg font-semibold mb-4 text-macos-primary">
            Input New Record
          </h3>
          <form
          ...
          >
            <div>
              ...
              <input
                ...
                type="text"
                required
                className="w-full bg-macos-tertiary border border-macos-separator text-macos-primary rounded-md p-2 text-sm focus:outline-none focus:border-macos-blue focus:ring-1 focus:ring-macos-blue transition"
              />
            </div>
            <div>
              <label className="block text-xs font-medium text-macos-secondary mb-1.5">
            ...
            <div>
              ...
              <input
                ...
                type="date"
                required
                className="w-full bg-macos-tertiary border border-macos-separator text-macos-primary rounded-md p-2 text-sm focus:outline-none focus:border-macos-blue focus:ring-1 focus:ring-macos-blue transition scheme-dark"
              />
            </div>
            <div>
              <label className="block text-xs font-medium text-macos-secondary mb-1.5">
            ...
            <div className="md:col-span-2 mt-2">
              <button
                type="submit"
                className="w-full py-2.5 bg-macos-blue text-white rounded-md font-semibold text-sm hover:bg-opacity-90 active:scale-[0.99] transition cursor-pointer shadow-lg"
              >
                Save Record
              </button>
            </div>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-4">
        ...
        <div className="flex items-center gap-3 w-full sm:w-auto">
          ...
          {query ? (
            ...
            <span className="text-sm font-medium text-macos-secondary opacity-40 cursor-not-allowed select-none whitespace-nowrap">
              Clear
            </span>
          )}
        </div>
            ...
            {records.length === 0 ? (
              ...
              records.map((record) => (
                ...
                >
                  ...
                  <td className="p-3.5 text-macos-secondary">
                    {new Date(record.dateDelivery).toLocaleDateString("id-ID")}
                  </td>
                  <td className="p-3.5 text-macos-secondary font-mono">
                    {record.noDo || "-"}
                  </td>
                  ...
                  <td className="p-3.5 text-right font-semibold text-macos-blue">
                    {formatIDR(record.amountIdr)}
                  </td>
                  <td className="p-3.5 pl-6 text-macos-secondary italic">
                    {record.remark || "-"}
                  </td>

                  {/* 🌟 TRIPLE DOT MENU 100% NATIVE CSS (Bebas dari Event Handler onClick) */}
                  <td className="p-3.5 text-center overflow-visible">
                    <div className="relative inline-block text-left group">
                      <span className="p-1 hover:bg-macos-tertiary rounded text-macos-secondary hover:text-macos-primary font-bold transition text-md leading-none select-none cursor-pointer">
                        •••
                      </span>

                      {/* Dropdown Menu melayang murni dikendalikan CSS group-hover */}
                      <div className="absolute right-0 hidden group-hover:block w-40 bg-macos-popover border border-macos-separator rounded-lg shadow-2xl z-50 py-1 overflow-hidden animate-scale-up text-left">
                        {/* Menggunakan elemen Link biasa (tanpa onClick) agar aman di server-side */}
                        <Link
                          href={`/dashboard/datapodo?invoice=${record.id}`}
                        ...
                        <form action={handleDelete} className="w-full">
                          ...
                          <button
                            type="submit"
                            className="w-full px-4 py-2 text-xs text-macos-red hover:bg-macos-red hover:text-white transition flex items-center gap-2 cursor-pointer font-medium"
                          >
                            Delete Record
                          </button>
                        </form>
