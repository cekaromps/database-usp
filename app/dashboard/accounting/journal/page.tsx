  ...
  return (
    <div className="p-6 max-w-6xl mx-auto space-y-6 min-h-screen bg-macos-base text-macos-primary">
      ...
      <div className="flex flex-col sm:flex-row justify-between sm:items-center gap-4">
        <div>
          <h1 className="text-2xl font-bold tracking-tight text-macos-primary">Jurnal Umum</h1>
          <p className="text-sm text-macos-secondary mt-1">Histori pencatatan kronologis seluruh transaksi keuangan.</p>
        </div>
        <Link 
      ...
      <div className="bg-macos-secondary rounded-lg shadow-xl border border-macos-separator overflow-hidden">
        {entries.length === 0 ? (
          <div className="p-12 text-center text-sm text-macos-tertiary italic">
            Belum ada transaksi yang dicatat. Silakan tambah jurnal baru terlebih dahulu.
          </div>
        ) : (
            ...
            <table className="w-full text-left text-sm text-macos-secondary min-w-[800px]">
              ...
              <thead className="bg-macos-popover border-b border-macos-separator text-xs text-macos-secondary uppercase font-semibold tracking-wider">
                <tr>
                  ...
                  <th className="px-6 py-3.5 w-36">Ref / No. Bukti</th>
                  <th className="px-6 py-3.5">Keterangan / Akun</th>
                  <th className="px-6 py-3.5 w-36 text-right">Debit (Rp)</th>
                  <th className="px-6 py-3.5 w-36 text-right">Kredit (Rp)</th>
              ...
              <tbody className="divide-y divide-macos-separator bg-macos-secondary">
                {entries.map((entry) => {
                  ...
                  return (
                    <tr key={entry.id} className="align-top hover:bg-macos-tertiary/40 transition">
                      ...
                      <td className="px-6 py-4 space-y-2">
                        <div className="font-semibold text-macos-primary text-base">{entry.description}</div>
                        ...
                        <div className="space-y-1 pl-2 border-l-2 border-macos-separator">
                          {entry.lines.map((line) => {
                            ...
                            return (
                              ...
                              >
                                <span className="col-span-3 font-mono text-macos-tertiary">[{line.account.code}]</span>
                                <span className="col-span-9">{line.account.name}</span>
                              </div>
                            );
                          })}
                        </div>
                      ...
                      <td className="px-6 py-4 text-right font-mono text-xs">
                        ...
                        <div className="invisible h-6 text-base">Header Spacer</div> 
                        <div className="space-y-1">
                          {entry.lines.map((line) => {
                            const debitNum = Number(line.debit);
                            return (
                              <div key={line.id} className="h-5 flex items-center justify-end text-macos-primary">
                                {debitNum > 0 ? debitNum.toLocaleString('id-ID', { minimumFractionDigits: 2 }) : "-"}
                              </div>
                            );
                          })}
                        </div>
                      ...
                      <td className="px-6 py-4 text-right font-mono text-xs">
                        ...
                        <div className="space-y-1">
                          {entry.lines.map((line) => {
                            ...
                            return (
                              <div key={line.id} className="h-5 flex items-center justify-end text-macos-secondary">
                                {creditNum > 0 ? creditNum.toLocaleString('id-ID', { minimumFractionDigits: 2 }) : "-"}
                              </div>
                            );
                          })}
                        </div>
                      ...
                      <td className="px-6 py-4 text-center whitespace-nowrap">
                        ...
                        <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-bold border ${
                          entry.status === 'POSTED' ? 'bg-macos-green/10 text-macos-green border-macos-green/20' :
                          entry.status === 'DRAFT' ? 'bg-macos-orange/10 text-macos-orange border-macos-orange/20' :
