...
export async function uploadExcelAction(formData: FormData) {
  ...
  try {
    ...
    if (!firstSheetName) {
      return {
        success: false,
        message: "Gagal mendeteksi nama Sheet di dalam file Excel.",
        inserted: 0,
        skipped: [],
      };
    }
    ...
    if (rawRows.length === 0) {
      return {
        success: false,
        message: "Berkas Excel yang diunggah terbaca hampa atau kosong.",
        inserted: 0,
        skipped: [],
      };
    }
    ...
    for (let i = 1; i < rawRows.length; i++) {
      ...
      if (excelCustomer && String(excelCustomer).trim() !== "")
        currentCustomer = String(excelCustomer).trim();
      if (excelQuotation && String(excelQuotation).trim() !== "")
        currentQuotation = String(excelQuotation).trim();
      if (excelNoPo && String(excelNoPo).trim() !== "")
        currentNoPo = String(excelNoPo).trim();
      if (excelNoDo && String(excelNoDo).trim() !== "")
        currentNoDo = String(excelNoDo).trim();
      if (excelNoInv && String(excelNoInv).trim() !== "")
        currentNoInv = String(excelNoInv).trim();
      ...
      if (
        cleanDesc.toLowerCase().includes("total") ||
        cleanDesc === "GRAND TOTAL" ||
        cleanDesc.toLowerCase().includes("jumlah")
      ) {
      ...
      if (!currentCustomer || currentCustomer === "UNKNOWN" || !currentNoInv) {
        skippedRowsReport.push({
          ...
          reason:
            "Kolom Customer Name atau Nomor Invoice hampa/tidak terdeteksi",
        });
        continue;
      }
      ...
      if (isDuplicateInDB) {
        skippedRowsReport.push({
          ...
          desc: cleanDesc,
          reason:
            "Data sudah pernah di-upload sebelumnya (Terdeteksi Duplikat)",
        });
      ...
      recordsToInsert.push({
        ...
        remark:
          excelRemark && String(excelRemark).trim() !== ""
            ? String(excelRemark).trim()
            : "-",
      });
    }
    ...
    if (recordsToInsert.length === 0) {
      return {
        success: false,
        message:
          "Tidak ada baris data valid yang bisa diimport. Pastikan urutan Kolom A sampai I sesuai format.",
        inserted: 0,
        skipped: skippedRowsReport,
      };
    }
    ...
    return {
      success: true,
      message: "Bulk import Excel berhasil!",
    ...
    return {
      success: false,
      message: "Gagal memproses berkas Excel karena error internal database.",
      inserted: 0,
      skipped: [],
    };
  }
...
export async function createInvoiceWithItemsAction(formData: FormData) {
  ...
  const rawRemark = formData.get("remark") as string;
  const remark =
    rawRemark && rawRemark.trim() !== ""
      ? rawRemark.trim()
      : "Prices are valid 1 month after offer is sent";
      ...
      recordsToInsert.map((record) =>
        prisma.invoice.create({
          data: {
            ...
            dateDelivery: record.dateDelivery,
            noInv: record.noInv,
...
export async function updateInvoiceAction(formData: FormData) {
  const noInv = formData.get("noInv") as string;
  const originalNoInv = formData.get("originalNoInv") as string;
  const quotationNumber = formData.get("quotationNumber") as string;
  const originalQuotationNumber = formData.get(
  ...
  const isRevision =
    noInv !== originalNoInv || quotationNumber !== originalQuotationNumber;

  try {
    // Pull the original rows first — we need them either to update,
    // or (if this is a revision) to carry over fields the form doesn't send
    // (address, processes, material, discount, unit).
    const originalItems = await prisma.invoice.findMany({
      where: { noInv: originalNoInv },
    });
    ...
    if (isRevision) {
      // =========================
      // REVISION: create a brand new invoice, leave the old one untouched
      // =========================
      let finalNoInv = noInv;
      if (noInv === originalNoInv) {
        const existingRevisions = await prisma.invoice.findMany({
          where: { noInv: { startsWith: `${originalNoInv}-R` } },
          select: { noInv: true },
          distinct: ["noInv"],
        });
        finalNoInv = `${originalNoInv}-R${existingRevisions.length + 1}`;
      }
        ...
        data: groupedItems
          ...
          .map((item) => {
            ...
            return {
              ...
              // carried over from the original row, since the form
              // doesn't expose these fields:
      ...
      // =========================
      // PLAIN EDIT (number unchanged): update in place, as before
      // =========================
    ...
    return isRevision
      ? "Failed to create revised invoice record"
...
export async function getNextSubItemNumber(customerName: string) {
  try {
    ...
    const cleanCustomer = customerName
      .replace("PT.", "")
      .replace("PT", "")
      .trim();
